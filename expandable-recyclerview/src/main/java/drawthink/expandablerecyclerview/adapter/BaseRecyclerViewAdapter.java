...
import com.drawthink.expandable_recyclerview.R;
...
public abstract class BaseRecyclerViewAdapter<T,S,VH extends BaseViewHolder> extends RecyclerView.Adapter<VH> {
    ...
    public void onBindViewHolder(final VH holder, final int position) {
        ...
        if (item != null && item instanceof GroupItem) {
            ...
            if(holder.img_orientation!=null) {
                if (selectArray.contains(gp)) {
                    System.out.println("=========selectArray=====01====="+gp);
                    holder.img_orientation.setImageResource(R.drawable.triangle02);
                }else{
                    System.out.println("=========selectArray=====02====="+gp);
                    holder.img_orientation.setImageResource(R.drawable.triangle);
                }
            }
    ...
    private int collapseGroup(int position) {
        ...
        if (((GroupItem) item).hasChilds()) {
            ...
            for(int j=0;j<tempChilds.size();j++){
                showingDatas.remove(position+1);
            }
